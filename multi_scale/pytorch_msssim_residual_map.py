...
def ssim(X, Y, 
    ...
    ssim_per_channel, cs, ssim_map = _ssim(X, Y,
...
def ms_ssim(X, Y, 
    ...
    for i in range(levels):
        ssim_per_channel, cs, ssim_map = _ssim(X, Y,
                                         ...
                                         size_average=False,
                                         K=K)
        residual_map.append(torch.relu(ssim_map))
        if i<levels-1: 
    ...
    mcs_and_ssim[mcs_and_ssim == 0] = 1e-12
    ms_ssim_val = torch.prod( mcs_and_ssim ** weights.view(-1, 1, 1), dim=0 )

    if size_average:
        return ms_ssim_val.mean(), residual_map
    else:
        return ms_ssim_val.mean(1), residual_map
